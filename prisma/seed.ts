...
async function main() {
  ...
  const admin = await prisma.user.create({ data: { id:"admin-id", name:"Administrator Utama", email:"admin@bapendamedan.go.id", password:adminPw, role:"ADMIN", nik:"1271000000000001", phone:"061-4567890", address:"Jl. Jenderal Ahmad Yani No. 1, Medan Baru, Kota Medan", isActive:true } });
  const admin2 = await prisma.user.create({ data: { id:"admin-id-2", name:"Kepala Bidang Pendataan", email:"kabid@bapendamedan.go.id", password:adminPw, role:"ADMIN", nik:"1271000000000003", phone:"08112345678", address:"Jl. Balai Kota No. 5, Medan Baru, Kota Medan", isActive:true } });
  ...
  console.log("🌱 Generating 30 Audit Logs...");
  const auditActions = [
    {action:"CREATE_TAX_OBJECT",table:"TaxObject",rec:"obj-pbb-1"},{action:"UPDATE_NJOP",table:"TaxObject",rec:"obj-pbb-2"},{action:"VERIFY_TAX_OBJECT",table:"TaxObject",rec:"obj-hotel-1"},
    {action:"CREATE_PAYMENT",table:"Payment",rec:"pay-0"},{action:"CONFIRM_PAYMENT",table:"Payment",rec:"pay-1"},{action:"APPROVE_KEBERATAN",table:"TaxSubmission",rec:"taxsub-1"},
    {action:"REJECT_PPID_REQUEST",table:"PPIDRequest",rec:"ppid-1"},{action:"APPROVE_RESEARCH",table:"ResearchRequest",rec:"research-1"},{action:"UPDATE_SYSTEM_SETTINGS",table:"SystemConfig",rec:"global-config"},
    {action:"CREATE_USER",table:"User",rec:"wp-user-10"},{action:"DEACTIVATE_USER",table:"User",rec:"wp-user-91"},{action:"EXPORT_TAX_DATA",table:"TaxObject",rec:"EXPORT-2026-06"},
    {action:"UPDATE_ZNT",table:"LandValueZone",rec:"Z-PETISAH-1"},{action:"CREATE_NEWS",table:"News",rec:"news-1"},{action:"PUBLISH_ANNOUNCEMENT",table:"Announcement",rec:"ann-1"},
    {action:"RESPOND_COMPLAINT",table:"Complaint",rec:"complaint-1"},{action:"GENERATE_SPPT",table:"Sppt",rec:"SPPT-2026-BATCH"},{action:"UPDATE_PROPERTY_MARKET",table:"PropertyMarket",rec:"pm-batch-2026"},
    {action:"RUN_TAX_ASSESSMENT",table:"TaxAssessment",rec:"assess-batch"},{action:"RESET_USER_PASSWORD",table:"User",rec:"wp-user-5"},{action:"LOGIN_ADMIN",table:"Session",rec:"sess-admin-001"},
    {action:"LOGIN_OFFICER",table:"Session",rec:"sess-officer-001"},{action:"BULK_IMPORT_TAX_OBJECTS",table:"TaxObject",rec:"IMPORT-2026-Q1"},{action:"DELETE_EXPIRED_PAYMENTS",table:"Payment",rec:"CLEANUP-2026"},
    {action:"APPROVE_PPID_REQUEST",table:"PPIDRequest",rec:"ppid-3"},{action:"UPDATE_USER_PROFILE",table:"User",rec:"wp-id-1"},{action:"GENERATE_LAPORAN_PAD",table:"Report",rec:"RPT-2026-Q2"},
    {action:"RESOLVE_COMPLAINT",table:"Complaint",rec:"complaint-5"},{action:"SYNC_BPN_DATA",table:"ObjectTaxLocation",rec:"SYNC-BPN-2026"},{action:"UPDATE_BUDGET_YEAR",table:"SystemConfig",rec:"global-config"},
  ];
  const auditUsers = [admin.id,admin2.id,officer.id,officer2.id,officer3.id];
  for (let i = 0; i < auditActions.length; i++) {
    const aa = auditActions[i];
    await prisma.auditLog.create({ data: { action:aa.action, table:aa.table, recordId:aa.rec, userId:auditUsers[i%auditUsers.length], oldValue:{status:"SEBELUMNYA",value:rand(100000,9999999)}, newValue:{status:"DIPERBARUI",value:rand(100000,9999999)}, createdAt:dateOffset(rand(0,170)) } });
  }
